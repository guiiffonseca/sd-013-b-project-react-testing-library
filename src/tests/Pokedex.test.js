...
describe(' Teste o componente <Pokedex.js />', () => {
  ...
  test('é mostrado apenas um Pokémon por vez.', () => {
    ...
    const pokemonsExibidos = screen.getAllByTestId('pokemon-name');
    expect(pokemonsExibidos.length).toBe(1);
  });
  ...
  test('se a Pokédex tem os botões de filtro', () => {
    ...
    const selectedPokemon = screen.getByTestId('pokemon-name');
    expect(selectedPokemon).toHaveTextContent(firePokemon[0].name);
    const buttonProximoPokemon = screen.getByRole('button', {
      name: /próximo pokémon/i,
    });
    userEvent.click(buttonProximoPokemon);
    expect(selectedPokemon).toHaveTextContent(firePokemon[1].name);
