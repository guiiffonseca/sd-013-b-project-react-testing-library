...
describe('Testando Pokedex.js', () => {
  ...
  test('a página deve conter um heading h2 com um texto específico', () => {
    renderWithRouter(
      <Pokedex pokemons={ pokemons } isPokemonFavoriteById={ { 4: true } } />,
    );
  ...
  test('a página deve exibir o próximo Pokémon da lista quando o botão é clicado', () => {
    renderWithRouter(
      <Pokedex
        pokemons={ [pokemons[0], pokemons[1], pokemons[2]] }
        isPokemonFavoriteById={ { 4: true, 143: true, 148: true } }
      />,
    );
